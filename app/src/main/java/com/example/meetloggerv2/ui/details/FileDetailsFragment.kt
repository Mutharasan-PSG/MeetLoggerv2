...
class FileDetailsFragment : Fragment() {
    ...
    private fun saveAsNewCopy() { FirebaseAuth.getInstance().currentUser?.uid?.let { uid -> fileName?.let { val name = "${it.substringBeforeLast(".")} (Copy).mp3"; viewModel.saveAsNewCopy(uid, name, viewModel.fileDetails.value?.toMutableMap()?.apply { put("fileName", name); put("Response", editText.text.toString()) } ?: mapOf()) } }; switchToViewMode() }
